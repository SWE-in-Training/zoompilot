...
void SpectraCamera::configICP() {
  ...
  */
  ...
  // bps_blobs.h is regenerated by comma and its AR0231 row has been an all zero
  // placeholder since the sensor was dropped. The cabin camera is the only
  // BPS processed path, and on a comma three it is an AR0231, so taking the
  // placeholder would hand the BPS a zeroed config and kill driver monitoring.
  // Keep the AR0231 blobs in their own header so a regeneration cannot clobber
  // them again.
...
bool SpectraCamera::processFrame(int buf_idx, uint64_t request_id, uint64_t ife_frame_id, uint64_t timestamp) {
  // The cabin camera's strobe is staggered half a period behind the others by the
  // panda's slave timer, which only exists on the H7 pandas. The comma three's dos
  // is an STM32F4: clock_source.h drives all three channels off TIM1 and never sets
  // the CCR4 half-period offset, so there is no stagger to correct for.
  const bool staggered = cc.staggered_sof && (Hardware::get_device_type() != cereal::InitData::DeviceType::TICI);
  if (!syncFirstFrame(cc.camera_num, request_id, ife_frame_id, timestamp, staggered)) {
    return false;
  }
